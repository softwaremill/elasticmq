...
* runs stand-alone ([download](https://s3-eu-west-1.amazonaws.com/softwaremill-public/elasticmq-server-0.14.15.jar)), via [Docker](https://hub.docker.com/r/softwaremill/elasticmq/) or embedded
...
[https://s3/.../elasticmq-server-0.14.15.jar](https://s3-eu-west-1.amazonaws.com/softwaremill-public/elasticmq-server-0.14.15.jar)
...
Simply run the jar and you should get a working server, which binds to `localhost:9324`:
    ...
    java -jar elasticmq-server-0.14.15.jar
...
configuration values, create a file (e.g. `custom.conf`), fill it in with the desired values, and pass it to the server:
    ...
    java -Dconfig.file=custom.conf -jar elasticmq-server-0.14.15.jar
...
log INFO logs and above to the console):
    ...
    java -Dlogback.configurationFile=my_logback.xml -jar elasticmq-server-0.14.15.jar
...
ENV ELASTICMQ_VERSION ${ELASTICMQ_VERSION:-0.14.15}
...
-----------------------------
    ...
    // Scala 2.12 and 2.11
    val elasticmqSqs        = "org.elasticmq" %% "elasticmq-rest-sqs" % "0.14.15"

If you don't want the SQS interface, but just use the actors directly, you can add a dependency only to the `core`
module:

    val elasticmqCore       = "org.elasticmq" %% "elasticmq-core" % "0.14.15"
...
*Stable*: 0.14.15, 0.8.12

*Development*: 0.14.15-SNAPSHOT
