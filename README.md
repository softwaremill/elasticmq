...
[https://s3/.../elasticmq-server-0.14.4.jar](https://s3-eu-west-1.amazonaws.com/softwaremill-public/elasticmq-server-0.14.4.jar)
...
Simply run the jar and you should get a working server, which binds to `localhost:9324`:
    ...
    java -jar elasticmq-server-0.14.4.jar
...
configuration values, create a file (e.g. `custom.conf`), fill it in with the desired values, and pass it to the server:
    ...
    java -Dconfig.file=custom.conf -jar elasticmq-server-0.14.4.jar
...
log INFO logs and above to the console):
    ...
    java -Dlogback.configurationFile=my_logback.xml -jar elasticmq-server-0.14.4.jar
...
*Stable*: 0.14.4, 0.8.12

*Development*: 0.14.4-SNAPSHOT
