...
* runs stand-alone ([download](https://s3-eu-west-1.amazonaws.com/softwaremill-public/elasticmq-server-0.9.2.jar)) or embedded
...
[https://s3/.../elasticmq-server-0.9.2.jar](https://s3-eu-west-1.amazonaws.com/softwaremill-public/elasticmq-server-0.9.2.jar)
...
Simply run the jar and you should get a working server, which binds to `localhost:9324`:
    ...
    java -jar elasticmq-server-0.9.2.jar
...
configuration values, create a file (e.g. `custom.conf`), fill it in with the desired values, and pass it to the server:
    ...
    java -Dconfig.file=custom.conf -jar elasticmq-server-0.9.2.jar
...
log INFO logs and above to the console):
    ...
    java -Dlogback.configurationFile=my_logback.xml -jar elasticmq-server-0.9.2.jar
...
Queues can be automatically created on startup by providing appropriate configuration (since 0.9.2):
...
-----------------------------
    ...
    // Scala 2.11
    val elasticmqSqs        = "org.elasticmq" %% "elasticmq-rest-sqs"         % "0.9.2"
...
module:
    ...
    val elasticmqCore       = "org.elasticmq" %% "elasticmq-core"             % "0.9.2"
