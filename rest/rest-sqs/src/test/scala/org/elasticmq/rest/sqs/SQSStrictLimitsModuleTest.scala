...
class SQSStrictLimitsModuleTest extends FunSuite with SQSLimitsModule {
  ...
  val correctValues = List(BigDecimal(10).pow(126), -BigDecimal(10).pow(128), BigDecimal(0), BigDecimal(Random.nextDouble())).map(v => v.toString)
  val incorrectValues = List((BigDecimal(10).pow(126) + 0.1).toString, (-BigDecimal(10).pow(128) - 0.1).toString, "12312312a")
