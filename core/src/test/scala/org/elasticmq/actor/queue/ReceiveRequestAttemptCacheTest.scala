...
class ReceiveRequestAttemptCacheTest extends FunSuite with Matchers{
	...
	test("should retrieve message by attempt id or return the appropriate failure") {
		...
		val msg1 = InternalMessage("id-1", mutable.Buffer.empty, 1L, "content", Map.empty, DateTime.now(), NeverReceived,
			receiveCount = 0, isFifo = false, messageGroupId = None, messageDeduplicationId = None)
